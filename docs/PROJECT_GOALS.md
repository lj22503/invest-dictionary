...
| F | Vercel 部署 + 域名绑定 dictionary.mangofolio.com | ✅ 部署成功（Git 集成，web/ 根目录）；域名已绑定并 live 验证（dictionary.mangofolio.com 正常访问；Cloudflare CNAME 已添加，Proxy 状态为橙云可选改 DNS only） |
